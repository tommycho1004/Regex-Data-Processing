...
for row in reader:
    if float(row[2]) > lv_thresh:
        ...
        if row[8] != 'NaN':
            ...
            hbc += 1
avg_atk_above = round(avg_atk_above/aac)
avg_atk_below = round(avg_atk_below/abc)
avg_def_above = round(avg_def_above/dac)
avg_def_below = round(avg_def_below/dbc)
avg_hp_above = round(avg_hp_above/hac)
avg_hp_below = round(avg_hp_below/hbc)
r = open('pokemonResult.csv', 'w',newline = '')
res = csv.writer(r,delimiter=',')
pf = open("pokemonTrain.csv", "r")
reader2 = csv.reader(pf)
